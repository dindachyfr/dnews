...
export const getServerSideProps = async () => {
  const categories = await fetch(`${process.env.NEXT_PUBLIC_BACKEND}/category?limit=5`, {
    method: 'GET'
  })
    ...
    <Layout>
      <main className={`container-fluid g-0 bg-cream ${styles.header}`}>
        <header className={`${styles.headerBG}`}>
          <div className="wrapper p-3 h-100 ps-5 w-50 d-flex flex-column justify-content-around">
            <h4 className={`text-blue ${styles.title}`}>Share Information and Educate People</h4>
            <h6 className="text-blue w-75">Everyone has their point of view of something, but just don’t be afraid to express the facts. Be an author and share you prespective of something to the world.</h6>
            <div
          ...
          <section className="category w-100 my-5">
            ...
            <div className="lower-category w-100 d-flex justify-content-between flex-wrap px-3">
              {cats?.map((cat, index) => {
                return (
                  <div
                    className={`${styles.wrapperCat} d-flex mx-3 flex-column pointer`}
                    key={index}
